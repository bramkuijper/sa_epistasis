...
if (!exists("the.data"))
{
    ...
    the.data[the.data$pAmf >= bound & the.data$pAmf <= 1-bound & the.data$pBmf >= bound & the.data$pBmf <= 1-bound
                & the.data$pAmm >= bound & the.data$pAmm <= 0.5-bound & the.data$pBmm >= bound & the.data$pBmm <= 0.5-bound,"state"] <- 1


    the.data[the.data$pAmf > 1-bound & the.data$pBmf > 1-bound & the.data$pAmm > .5-bound & the.data$pBmm > .5-bound,"state"] <- 2


    the.data[the.data$pAff > 1-bound & the.data$pBff > 1-bound & the.data$pAfm > .5-bound & the.data$pBfm > .5-bound,"state"] <- 3


    the.data[(
                ((the.data$pAff > 1-bound | the.data$pAff < bound) & the.data$pBff >= bound & the.data$pBff <= 1-bound) | 
                ((the.data$pBff > 1-bound | the.data$pBff < bound) & the.data$pAff >= bound & the.data$pAff <= 1-bound)
             )& (
                ((the.data$pAfm > .5-bound | the.data$pAfm < bound) & the.data$pBfm >= bound & the.data$pBfm <= .5 - bound) | 
                ((the.data$pBfm > .5-bound | the.data$pBfm < bound) & the.data$pAfm >= bound & the.data$pAfm <= .5 - bound)
            ), "state"] <- 4


    the.data[(the.data$pAff > 1-bound & the.data$pBff < bound)
                &
            (the.data$pAfm > .5 -bound & the.data$pBfm < bound),"state"] <- 5


    the.data[(the.data$pAff < bound & the.data$pBff > 1-bound)
                & 
                (the.data$pAfm < bound & the.data$pBfm > .5-bound),"state"] <- 6
}
